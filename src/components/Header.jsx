  ...
  return (
    <header className='bg-orange-500 text-white dark:bg-black dark:text-white'>
      <nav className='p-4 flex justify-between gap-6 place-items-center max-w-7xl mx-auto'>
        ...
        <ul className='md:flex place-items-center gap-12 text-lg hidden'>
          {navItems.map(({ path, link }) => (
            <Link to={path} className='text-white' key={path}>
              {link}
            </Link>
          ))}
          {/* button */}
      ...
      <div className='text-black'>
        <ul
          className={`md:hidden block text-lg space-y-4 px-4 py-6 mt-14 bg-white ${
            isOpen
              ? 'fixed top-0 left-0 w-full transition-all ease-out duration-100'
              : 'hidden'
          }`}
        >
          {navItems.map(({ path, link }) => (
            <li className='text-red-500' key={path}>
              {link}
            </li>
          ))}
        </ul>
